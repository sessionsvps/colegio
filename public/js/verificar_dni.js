...
document.getElementById("boton_verificar2").addEventListener("click", traerDatos2);
...
function mostrarMensaje2(tipo, mensaje) {
...
function traerDatos2() {
    ...
    .then((datos) => {
        if (datos.estado) {
            ...
            mostrarMensaje2('green', 'Verificación hecha correctamente');
        } else {
            limpiarCampos();
            mostrarMensaje2('red', 'No se encontró a una persona con ese DNI');
        }
    })
    .catch((error) => {
        console.error('Error:', error);
        limpiarCampos2();
        mostrarMensaje2('red', 'Hubo un problema al buscar los datos. Inténtalo de nuevo más tarde.');
    });
}
