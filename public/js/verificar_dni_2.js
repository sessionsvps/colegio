document
    .getElementById("boton_verificar2")
    .addEventListener("click", function () {
        ...
        fetch(`/apoderados/buscar?dni=${dni}`)
            ...
            .then((data) => {
                if (data.success) {
                    ...
                    document.getElementById("fecha_nacimiento_ap").value =
                        apoderado.fecha_nacimiento.split(" ")[0];
                    document.getElementById("sexo_texto_ap").value =
                        apoderado.sexo ? "Masculino" : "Femenino";
                    document.getElementById("sexo_ap").value = apoderado.sexo
                        ? "1"
                        : "0";
                    ...
                    document
                        .getElementById("email_ap")
                        .setAttribute("readonly", true);
                    document
                        .getElementById("telefono_celular_ap")
                        .setAttribute("readonly", true);
                    // Mostrar la foto
                    if (photoUrl) {
            ...
            .catch((error) => {
                ...
                mostrarMensaje_ap(
                    "red",
                    "Hubo un problema al buscar los datos en la base de datos."
                );
            });
    });
    ...
    fetch(
        ...
        .then((datos) => {
            if (datos.estado) {
                ...
                document.getElementById("otros_nombres_ap").value =
                    resultado.nombres.split(" ").slice(1).join(" ");
                document.getElementById("apellido_paterno_ap").value =
                    resultado.apellido_paterno;
                document.getElementById("apellido_materno_ap").value =
                    resultado.apellido_materno;
                const genero =
                    resultado.genero === "M" ? "Masculino" : "Femenino";
                document.getElementById("sexo_texto_ap").value = genero;
                document.getElementById("sexo_ap").value =
                    resultado.genero === "M" ? "1" : "0";
                document.getElementById("fecha_nacimiento_ap").value =
                    resultado.fecha_nacimiento.split("/").reverse().join("-"); // Convertir a formato YYYY-MM-DD
                mostrarMensaje_ap("green", "Verificación hecha correctamente");
            } else {
                limpiarCampos_ap();
                mostrarMensaje_ap(
                    "red",
                    "No se encontró a una persona con ese DNI"
                );
            }
        })
        .catch((error) => {
            ...
            mostrarMensaje_ap(
                "red",
                "Hubo un problema al buscar los datos. Inténtalo de nuevo más tarde."
            );
        });
}
