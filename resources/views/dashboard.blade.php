            ...
            <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="min-w-full rounded-lg border-b border-gray-200 shadow sm:rounded-lg bg-white">
                    <form method="POST" action="{{ route('dashboard.notas-alumnos-por-seccion') }}">
                        @csrf
                        <div class="my-5 md:my-10 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                            <div>
                                ...
                                <select id="nivel" name="nivel" onchange="updateGrados()" required
                                    ...
                                    @foreach ($niveles as $nivel)
                                        <option value="{{ $nivel->id_nivel }}"
                                            {{ request('nivel') == $nivel->id_nivel ? 'selected' : '' }}>
                                            {{ $nivel->detalle }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select id="bimestre" name="bimestre"
                                    ...
                                    @foreach ($bimestres as $bimestre)
                                        <option value="{{ $bimestre->id }}"
                                            {{ request('bimestre') == $bimestre->id ? 'selected' : '' }}>
                                ...
                                <table class="min-w-full bg-white">
                                    <thead class="bg-gray-100">
                                        <tr>
                                            ...
                                            <th
                                                class="py-2 px-4 border-b border-gray-200 text-center text-sm font-semibold text-gray-600">
                                                Nivel de Logro A
                                            </th>
                                            <th
                                                class="py-2 px-4 border-b border-gray-200 text-center text-sm font-semibold text-gray-600">
                                    ...
                                    <tbody>
                                        @foreach ($logrosPorGradoSeccion as $item)
                                            <tr>
                                                <td
                                                    class="py-2 px-4 border-b border-gray-200 text-left text-sm text-gray-700">
                                                    {{ $item['grado_seccion'] }}
                                                </td>
                                                <td
                                                    class="py-2 px-4 border-b border-gray-200 text-center text-sm text-gray-700">
                                                    {{ $item['nivel_logro_A'] }}
                                                </td>
                                                <td
                                                    class="py-2 px-4 border-b border-gray-200 text-center text-sm text-gray-700">
                ...
                <div class="min-w-full rounded-lg border-b border-gray-200 shadow sm:rounded-lg bg-white pt-10">
                    <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="md:mt-6" id="botonFiltrar">
...
@section('scripts')
    @if (Auth::user()->hasRole('Director') || Auth::user()->hasRole('Secretaria') || Auth::user()->hasRole('Admin'))
        <script>
            let chart3;
            let chart4;
            am5.ready(function() {
