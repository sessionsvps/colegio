...
@section('contenido')
    ...
    <div>
        ...
        <form method="GET" action="{{ route('catedras.index') }}">
            <div class="my-5 md:my-10 grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-4 lg:gap-4">
                <div class="">
                        ...
                        @else
                            @if ($filtra_nivel == 1)
                                @foreach ($grados_primaria as $grado_p)
                                    <option value="{{ $grado_p->id_grado }}"
                                        {{ request('grado') == $grado_p->id_grado ? 'selected' : '' }}>
                                        {{ $grado_p->detalle }}
                                    </option>
                                @endforeach
                            @elseif($filtra_nivel == 2)
                                @foreach ($grados_secundaria as $grado_s)
                                    <option value="{{ $grado_s->id_grado }}"
                                        {{ request('grado') == $grado_s->id_grado ? 'selected' : '' }}>
                                        {{ $grado_s->detalle }}
                                    </option>
                                @endforeach
                            @endif
                ...
                <div class="mt-3 md:mt-0" id="botonBuscar">
                    <button type="submit"
            ...
            <input type="text" id="año_escolar" name="año_escolar" class="hidden" value="{{ request('año_escolar') }}"
                readonly>
        </form>
