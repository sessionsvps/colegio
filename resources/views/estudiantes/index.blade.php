            ...
            <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <select id="filtrar_por" name="filtrar_por"
                        ...
                        <option value="matriculado" {{ request('filtrar_por') == 'matriculado' ? 'selected' : '' }}>
                            Matriculado
                        </option>
            ...
            <tbody>
                @forelse ($estudiantes as $estudiante)
                    <tr
                        ...
                        @if (Auth::user()->hasRole('Apoderado'))
                            <td class="px-6 py-4">
                                {{ $estudiante->seccion->grado->detalle }} {{ $estudiante->seccion->detalle }} de
                                {{ $estudiante->seccion->grado->nivel->detalle }}
                            </td>
                        @endif
                        <td class="px-6 py-4">
                            @if (Auth::user()->hasRole('Apoderado'))
                        ...
                        @if (!Auth::user()->hasRole('Apoderado'))
                            <td class="px-6 py-4">
                                {{ $estudiante->apoderado->primer_nombre }} {{ $estudiante->apoderado->otros_nombres }}
                                {{ $estudiante->apoderado->apellido_paterno }}
